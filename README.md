...
安装时请同时完成：
1. 把 Info-Alchemist 放到当前 AI Agent 可以读取的位置。
2. 创建或检查 `.env` 配置文件。
3. 把 `references/host_integration.md` 里的 Info-Alchemist 接入规则写入当前工作区的 Agent 规则里。

注意：不要只下载文件。只有当前 Agent 知道什么时候调用 Info-Alchemist，才算安装完成。

安装完成后，请告诉我是否完成，以及我第一次触发时会在哪里填写 Tavily API key。
```

第一次触发 Info-Alchemist 时，如果还没有填写 Tavily API key，AI Agent 会给你一个本地配置页面。打开页面填写后，回到 AI Agent 重新发送刚才的问题。
...
4. 复制以 `tvly-` 开头的 key。
5. 第一次触发 Info-Alchemist 时，打开 AI Agent 给出的本地配置页面。
6. 把 key 填到 Tavily API key 输入框里。
7. 保存配置。

## TikHub API key 怎么拿

TikHub 是可选项。只有当你想查小红书、X、Reddit 上的用户讨论、抱怨、口碑和社区信号时，才需要配置。

1. 打开 https://www.tikhub.io/
2. 注册或登录。
3. 找到 API key、Token 或控制台里的密钥页面。
4. 复制你的 TikHub API key。
5. 打开 Info-Alchemist 的本地配置页面。
6. 打开“社交平台搜索”开关。
7. 把 key 填到 TikHub API key 输入框里。
8. 保存配置。

如果你不需要社交平台搜索，就保持 `INFO_ALCHEMIST_ENABLE_TIKHUB=0`，不用填写 `TIKHUB_API_KEY`。
...
运行后，你可能会在当前工作区或安装目录附近看到一个 `info-alchemist/` 文件夹。这里保存本地报告、运行日志、缓存和个人决策画像。

## 如果没有成功

| 现象 | 怎么办 |
|---|---|
| AI Agent 说缺少 Tavily API key | 打开它给出的本地配置页面，填入 Tavily API key |
