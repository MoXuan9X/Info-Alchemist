...
这个公开版本在用户本地运行，使用者需要自备 Tavily API key；TikHub 是可选补充源。

```bash
git clone https://github.com/MoXuan9X/Info-Alchemist.git Info-Alchemist
cd Info-Alchemist
cp .env.example .env
```
...
如果宿主支持本地 Agent Skills，把整个 `Info-Alchemist/` 目录放进宿主的 skills 目录，让宿主按 `SKILL.md` 自动发现。
